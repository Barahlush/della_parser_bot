...
def make_short_search_query_params(
    ...
    """Generates short search query params

    Args:
        cal_input_text_from (str): Date of the route start
        cal_input_text_to (str): Date of the route end
        classic_cargo (bool): Is classic cargo or transport
        truck_select (list[str]): List of truck type ids
        multisearch_block_from (list[list[str]]): List of route start points
        multisearch_block_to (list[list[str]]): List of route end points
        is_not_deleted_filter (bool): Is not deleted filter
        is_customer_filter (bool): Is customer filter
        is_partner_filter (bool): Is partner filter
        is_not_black_filter (bool): Is not black filter
        is_load_up_filter (bool): Is load up filter
        is_adr_filter (bool): Is adr filter
        is_trans_with_gps_filter (bool):
        is_humanitarian_filter (bool):
        is_only_new_filter (bool):
        is_only_from_verified_filter (bool):
        ...
        load_type_filter (list[str]):
        bookmark_number (str):
        weight_from (str, optional): . Defaults to '0'.
        weight_to (str, optional): . Defaults to '0'.
        cube_from (str, optional): . Defaults to '0'.
        cube_to (str, optional): . Defaults to '0'.
...
def make_short_search_query_url(
    cal_input_text_from: str,
    cal_input_text_to: str,
    truck_select: list[str],
    multisearch_block_from: list[list[str]],
    ...
    params = make_short_search_query_params(
        ...
        classic_cargo,
        truck_select,
        multisearch_block_from,
        multisearch_block_to,
        is_not_deleted_filter,
        is_customer_filter,
        is_partner_filter,
        is_not_black_filter,
        is_load_up_filter,
